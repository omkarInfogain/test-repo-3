...
def render(project_root: Path, skill_dir: Path) -> Path:
    ...
    # Create _bmad/ if absent so render output directory can be written
    (project_root / "_bmad").mkdir(exist_ok=True)

    sources = _load_sources(skill_dir)
    # Fall back to bundled config when project has no _bmad/config.toml
    central = load_central_config(project_root, fallback=Path(__file__).parent / "config.toml")
