...
1. Explicit argument
   ...
   <!-- H.Singh 2026-08-18: Added planning artifact story detection so stories from the planning folder are routed correctly through step-02 rather than treated as unstructured intent. A raw planning story has no Code Map or Tasks & Acceptance, so it must never skip to step-03. -->
   - If it points to a file that contains **all three** of these bold-header markers in its body — `**Story ID:**`, `**Status:**`, and either `**Epic:**` or `**Story**` — it is a **planning artifact story**. Extract:
     - `epic_num` and `story_num` from `**Story ID:**` (e.g. `STORY-19-5` → epic 19, story 5) and from `**Epic:**` as a cross-check.
     - Status from the `**Status:**` line: map `✅ Done` / `Implemented` → `done`; `In Progress` / `🚧` → `in-progress`; `Ready for Dev` / `✅ Ready` → `ready-for-dev`; anything else → treat as `draft`.
     - For `done`: ingest as context and proceed to INSTRUCTIONS — do not resume.
     - For any other status: ingest as starting intent and proceed directly to **INSTRUCTIONS → epic path A** using the extracted `epic_num` and `story_num`. Do NOT early-exit to step-03 — the planning story has no Code Map, no `context:` list, and no task checkboxes; step-02 must create a proper spec first.
