...
3. Read `[[bmad-snapshot:spec-template.md]]` fully. Fill it out based on the intent and investigation, resolving the template's `date` field to the current system date. Drain the investigation into the `## Code Map` section — annotated paths, symbol/line anchors, reuse pointers, and read-only evidence — so the spec is the implementer's investigation map and the step-03 handoff need only point at it. If `preserved_intent` is non-empty, replace the `<frozen-after-approval>` block in the spec you just filled out with `preserved_intent`, before writing. Write the result to `{spec_file}`.
   <!-- H.Singh 2026-08-18: When source intent is a planning artifact story, auto-derive Tasks & Acceptance and context: from its structured table rather than inferring from scratch, to keep the spec faithful to the story's own file plan. -->
   - If the source intent is a planning artifact story (body contains a file table with `| File | Action |` or `File Locations Summary` columns): after filling the spec template, (a) populate `## Tasks & Acceptance` checkboxes by converting each table row into one `[ ]` task (file path — action — AC reference); (b) populate the `context:` frontmatter field with every file whose Action is `Edit`, `Modify`, or `Update` — these are the existing files the implementer must read before touching anything.
