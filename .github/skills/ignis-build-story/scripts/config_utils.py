...
def load_central_config(project_root: Path, fallback: "Path | None" = None) -> dict[str, Any]:
    bmad_dir = project_root / "_bmad"
    config_path = bmad_dir / "config.toml"
    # Use bundled fallback when project has no _bmad/config.toml
    base = load_toml(config_path if config_path.exists() else (fallback or config_path), required=True)
    return merge_layers(
        (
            base,
