...
<!-- H.Singh 2026-08-18: Defensive fallback for specs that have no task checkboxes (e.g. a planning story routed here without going through step-02). Avoids an ambiguous halt. -->
- **Fallback:** If `## Tasks & Acceptance` is absent or contains no `[ ]` tasks, verify completion against every AC in `{spec_file}` instead — treat each AC as a task, confirm it is satisfied, and finish any that are not before proceeding.
